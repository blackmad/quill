  ...
  "query" - {
    ...
    "map" - {
      ...
      "dynamic" in {
        var cond = true
        test(
          dynamicQuery[TestEntity].map(v0 => if (cond) v0.i else 1),
          query[TestEntity].map(v0 => v0.i)
        )
    ...
    "flatMap" - {
      ...
      "with map" in {
        test(
          dynamicQuery[TestEntity].flatMap(v0 => dynamicQuery[TestEntity].map(v1 => (unquote(v0), unquote(v1)))),
    ...
    "sortBy" in {
      test(
        dynamicQuery[TestEntity].sortBy(v0 => v0.i)(Ord.desc[Int]),
